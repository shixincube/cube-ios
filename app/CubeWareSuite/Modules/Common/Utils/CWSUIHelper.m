...
#import "CWSUIHelper.h"

@implementation CWSUIHelper
...
@implementation CWSUIHelper (QMUIMoreOperationAppearance)

+ (void)customMoreOperationAppearance {
    // 如果需要统一修改全局的 QMUIMoreOperationController 样式，在这里修改 appearance 的值即可
    [QMUIMoreOperationController appearance].cancelButtonTitleColor = UIColor.cws_tintColor;
}

@end

@implementation CWSUIHelper (QMUIAlertControllerAppearance)
...
@implementation CWSUIHelper (QMUIDialogViewControllerAppearance)
...
+ (void)customDialogViewControllerAppearance {
    ...
    appearance.backgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<CWSThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:CWSThemeIdentifierDark]) {
            return UIColorMake(34, 34, 34);
        }
        return UIColorWhite;
    }];
    appearance.headerViewBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<CWSThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:CWSThemeIdentifierDark]) {
            return UIColorMake(34, 34, 34);
        }
    ...
    appearance.headerSeparatorColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<CWSThemeProtocol> * _Nullable theme) {
        if ([identifier isEqualToString:CWSThemeIdentifierDark]) {
    ...
    buttonTitleAttributes[NSForegroundColorAttributeName] = UIColor.cws_tintColor;
    appearance.buttonTitleAttributes = [buttonTitleAttributes copy];
    
    appearance.buttonHighlightedBackgroundColor = [UIColor.cws_tintColor colorWithAlphaComponent:.25];
}

@end

@implementation CWSUIHelper (QMUIEmotionView)
...
+ (void)customEmotionViewAppearance {
    ...
    [QMUIEmotionView appearance].sendButtonBackgroundColor = UIColor.cws_tintColor;
}

@end

@implementation CWSUIHelper (QMUIImagePicker)
...
+ (void)customImagePickerAppearance {
    ...
    [QMUIImagePickerCollectionViewCell appearance].checkboxImage = [checkboxImage qmui_imageWithTintColor:UIColor.cws_tintColor];
    [QMUIImagePickerCollectionViewCell appearance].checkboxCheckedImage = [checkboxCheckedImage qmui_imageWithTintColor:UIColor.cws_tintColor];
    [QMUIImagePickerPreviewViewController appearance].toolBarTintColor = UIColor.cws_tintColor;
}

@end

@implementation CWSUIHelper (QMUIPopupContainerView)

+ (void)customPopupAppearance {
    QMUIPopupContainerView *popup = QMUIPopupContainerView.appearance;
    popup.backgroundColor = UIColor.cws_backgroundColor;
    popup.borderColor = UIColor.cws_separatorColor;
    popup.maskViewBackgroundColor = [UIColor qmui_colorWithThemeProvider:^UIColor * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, __kindof NSObject<CWSThemeProtocol> * _Nullable theme) {
        return [identifier isEqual:CWSThemeIdentifierDark] ? UIColorMask : UIColorMaskWhite;
    }];
    
    QMUIPopupMenuView *menuView = QMUIPopupMenuView.appearance;
    menuView.itemSeparatorColor = UIColor.cws_separatorColor;
    menuView.sectionSeparatorColor = UIColor.cws_separatorColor;
    menuView.itemTitleColor = UIColor.cws_tintColor;
}

@end

@implementation CWSUIHelper (UITabBarItem)
...
@implementation CWSUIHelper (Button)
...
+ (QMUIButton *)generateDarkFilledButton {
    ...
    button.backgroundColor = UIColor.cws_tintColor;
    button.highlightedBackgroundColor = [UIColor.cws_tintColor qmui_transitionToColor:UIColorBlack progress:.15];// 高亮时的背景色
...
+ (QMUIButton *)generateLightBorderedButton {
    QMUIButton *button = [[QMUIButton alloc] qmui_initWithSize:CGSizeMake(200, 40)];
    button.titleLabel.font = UIFontBoldMake(14);
    button.tintColorAdjustsTitleAndImage = UIColor.cws_tintColor;
    button.backgroundColor = [UIColor.cws_tintColor qmui_transitionToColor:UIColorWhite progress:.9];
    button.highlightedBackgroundColor = [UIColor.cws_tintColor qmui_transitionToColor:UIColorWhite progress:.75];// 高亮时的背景色
    button.layer.borderColor = [button.backgroundColor qmui_transitionToColor:UIColor.cws_tintColor progress:.5].CGColor;
    button.layer.borderWidth = 1;
    button.layer.cornerRadius = 4;
    button.highlightedBorderColor = [button.backgroundColor qmui_transitionToColor:UIColor.cws_tintColor progress:.9];// 高亮时的边框颜色
    return button;
}

@end

@implementation CWSUIHelper (Emotion)
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        for (QMUIEmotion *e in emotions) {
            e.image = [UIImageMake(e.identifier) qmui_imageWithBlendColor:UIColor.cws_tintColor];
        }
    });
}
...
@implementation CWSUIHelper (SavePhoto)
...
@implementation CWSUIHelper (Calculate)
...
@implementation CWSUIHelper (Theme)
