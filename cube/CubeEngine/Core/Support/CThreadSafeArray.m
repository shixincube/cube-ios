...
//  CServiceSuite
//
//  Created by Ashine on 2020/9/10.
//  Copyright © 2020 Ashine. All rights reserved.
//

/*
 * reference from YYKit  https://github.com/ibireme/YYKit
 */
...
@implementation CThreadSafeArray {
...
- (NSArray *)sortedArrayUsingComparator:(NS_NOESCAPE NSComparator)cmptr {
    LOCK(NSArray * a = [_arr sortedArrayUsingComparator:cmptr]); return a;
}

- (NSArray *)sortedArrayWithOptions:(NSSortOptions)opts usingComparator:(NS_NOESCAPE NSComparator)cmptr {
    LOCK(NSArray * a = [_arr sortedArrayWithOptions:opts usingComparator:cmptr]); return a;
}

- (NSUInteger)indexOfObject:(id)obj inSortedRange:(NSRange)r options:(NSBinarySearchingOptions)opts usingComparator:(NS_NOESCAPE NSComparator)cmp {
...
- (void)sortUsingComparator:(NS_NOESCAPE NSComparator)cmptr {
    LOCK([_arr sortUsingComparator:cmptr]);
}

- (void)sortWithOptions:(NSSortOptions)opts usingComparator:(NS_NOESCAPE NSComparator)cmptr {
    LOCK([_arr sortWithOptions:opts usingComparator:cmptr]);
}

- (BOOL)isEqualToArray:(NSArray *)otherArray {
    if (otherArray == self) return YES;
    if ([otherArray isKindOfClass:CThreadSafeArray.class]) {
        CThreadSafeArray *other = (id)otherArray;
        BOOL isEqual;
...
- (BOOL)isEqual:(id)object {
    ...
    if ([object isKindOfClass:CThreadSafeArray.class]) {
        CThreadSafeArray *other = object;
        BOOL isEqual;
        dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
        dispatch_semaphore_wait(other->_lock, DISPATCH_TIME_FOREVER);
